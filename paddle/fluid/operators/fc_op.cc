...
framework::OpKernelType FCOp::GetExpectedKernelType(
  ...
  if (ctx.Attr<bool>("use_mkldnn")) {
    ...
    customized_type_value = (input_data_type == framework::DataTypeTrait<int8_t>::DataType || input_data_type == framework::DataTypeTrait<uint8_t>::DataType)? kFCMKLDNNINT8 : kFCMKLDNNFP32;    
  }
  return framework::OpKernelType(input_data_type,
                                 ctx.GetPlace(), layout, library, customized_type_value);
}
...
void FCOpMaker::Make() {
  ...
  AddAttr<std::vector<float>>("Scale_weights",
                 "(std::vector<float>, default {1.0f}), The quantize scale of weights data")
		 .SetDefault({1.0f});
