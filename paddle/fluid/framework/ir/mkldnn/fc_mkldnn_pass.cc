...
std::unique_ptr<ir::Graph> FCMKLDNNPass::ApplyImpl(
  ...
  auto handler = [&](const GraphPatternDetector::subgraph_t& subgraph,
    ...
    OpDesc* desc = fc->Op();
    int input_index = 0;
    for(auto var_name: desc->InputNames()) {
      if (var_name == "Input") break;
      input_index++;

    }
    auto in_size = fc->inputs[input_index]->Var()->GetShape().size();
    ...
    // TODO[guomingz]:it's not a perfect solution
    if(weights->Name().find("transposed") == -1) {
